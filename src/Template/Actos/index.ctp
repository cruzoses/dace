    ...
    <div class="col-xs-12">
        <div class="box box-sace box-solid">
            <div class="box-header with-border">
                <h3 class="box-title"><i class="fa fa-book"></i>&nbsp;Lista de Actos</h3>
            ...
            <div class="box-body table-responsive no-padding">
		        <div class="oculto" id="buscar">
			        <?= $this->element('search_form', ['title' => 'Buscar Acto', 'searchFields' => $searchFields, 'filtros' => $filtros]);?>
		        </div>
                <table class="table table-bordered table-hover table-condensed">
                    <thead>
                        <tr>
                            <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                            <th scope="col"><?= $this->Paginator->sort('nombre','Nombre de la Promoción') ?></th>
                            <th scope="col" class="text-center"><?= $this->Paginator->sort('cohorte') ?></th>
                            <th scope="col" class="text-center"><?= $this->Paginator->sort('lapso') ?></th>
                            <th scope="col" class="text-center"><?= $this->Paginator->sort('fecha') ?></th>
                    ...
                    <tbody>
                        <?php foreach ($actos as $acto): ?>
                            <?php $sClass = $acto->activo ? 'label-success' : 'label-danger'; ?>
                            <tr>
                                <td><?= $this->Number->format($acto->id) ?></td>
                                <td><?= h($acto->nombre) ?></td>
                                <td class="text-center"><?= h($acto->cohorte) ?></td>
                                <td class="text-center"><?= $this->Number->format($acto->lapso) ?></td>
                                ...
                                <td class="text-center"><?= h($acto->modified) ?></td>
                                <td class="actions text-center">
                                    <?= $this->Html->link('<i class="fa fa-eye"></i>', ['action' => 'view', $acto->id], ['class'=>'btn btn-warning btn-xs','escape' => false]) ?>
                                    <?= $this->Html->link('<i class="fa fa-edit"></i>', ['action' => 'edit', $acto->id], ['class'=>'btn btn-info btn-xs','escape' => false]) ?>
                                    <?= $this->Form->postLink('<i class="fa fa-trash"></i>', ['action' => 'delete', $acto->id], ['confirm' => __('Are you sure you want to delete # {0}?', $acto->id), 'class'=>'btn btn-danger btn-xs','escape' => false]) ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
