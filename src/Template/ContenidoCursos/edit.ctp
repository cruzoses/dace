<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\ContenidoCurso $contenidoCurso
 */
?>
<div class="row">
    <div class="col-md-12">    
        <div class="box box-purple box-solid">
            <div class="box-header with-border">
                <h3 class="box-title"><i class="fa fa-book"></i>&nbsp;Editar Plan de Evaluaci&oacute;n</h3>
		        <div class="box-tools pull-right">
			        <?= $this->Html->link('<i class="fa fa-times"></i>',
				        ['action' => 'index', $nCursoId],
				        ['class'=>'btn btn-box-tool','title'=>'cerrar','escape'=>false]);
			        ?>
		        </div>
            </div>                    
            <?= $this->Form->create($contenidoCurso, [
                'role' => 'form', 
                'align' => [
                    'sm' => ['left' => 6, 'middle' => 6, 'right' => 12],
                    'md' => ['left' => 2,'middle' => 9,'right' => 1]
                ],
                'class' => 'horizontal']); 
            ?>
            <div class="box-body">
                <?php
                    echo $this->Form->control('fecha', ['label' => 'Fecha', 'type' => 'text', 
                        'class' => 'calendario', 'append' => '<i class="far fa-calendar-alt"></i>',
                        'prepend' => '<i class="fa fa-asterisk"></i>']
                    );
                    echo $this->Form->control('descripcion', ['label' => 'Título',
                        'class' => 'isUpper', 'prepend' => '<i class="fa fa-asterisk"></i>']
                    );
                    echo $this->Form->control('detalle', ['label' => 'Descripción',
                        'class' => 'isUpper', 'rows' => 2, 'prepend' => '<i class="fa fa-asterisk"></i>']
                    );
                    echo $this->Form->control('ponderacion', ['type' => 'select', 'label' => 'Ponderación',
                        'options' => $aPorcentajes, 'empty' => true,
                        'class' => 'form-control select2', 'data-width' => '100%',
                        'prepend' => '<i class="fa fa-asterisk"></i>']
                    );
                    echo $this->Form->control('indicador_curso_id', ['label' => 'Vincular con', 'type' => 'select',
                        'options' => $indicadorCursos, 'empty' => true,
                        'class' => 'form-control select2', 'data-width' => '100%',
                        'prepend' => '<i class="fa fa-asterisk"></i>']
                    );
                    echo $this->Form->control('activo', ['label' => 'Activo',
                        'type' => 'select', 'options' => [1 => 'Sí', 0 => 'No'],
                        'class' => 'form-control select2', 'data-width' => '100%',
                        'prepend' => '<i class="fa fa-asterisk"></i>']
                    );
                ?>
            </div>            
            <div class="box-footer">
		        <?= $this->Form->button('<i class="fa fa-save"></i>&nbsp;Guardar',
			        ['type' => 'submit','class'=>'btn btn-success btn-flat pull-left','escape'=>false]); 
		        ?>
		        <?= $this->Html->link('<i class="fa fa-power-off"></i>&nbsp;Cerrar',
			        ['action' => 'index', $nCursoId],
                    ['class' => 'btn bg-maroon btn-flat pull-right','escape' => false]); 
		        ?>
            </div>
            <?php echo $this->Form->end(); ?>
        </div>
    </div>
</div>
